...
do
	...
	((echo N$j.$i $N1 slow && ../material_P3/./slow $N1 | grep time && echo fast && ../material_P3/./fast $N1 | grep time) | echo $(cat $1)) | awk '{print $2, $6, $10}' >> time_slow_fast.dat
	N1=$((1024 + N1))
done
...
gnuplot << EOF
    ...
    plot '< sort -nk1 time_slow_fast.dat' using 1:2 title 'slow' with lines, \
        '< sort -nk1 time_slow_fast.dat' using 1:3 title 'fast' with lines
    quit
EOF
